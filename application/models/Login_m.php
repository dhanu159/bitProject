...
//        $sql =  $this->db->query("SELECT varEmail FROM user WHERE varEmail= '$email' and passWord = '$pwd'");
        $query = $this->db->query("SELECT u.varEmail,r.varRole,e.varEmpFname FROM user_has_role1 uhr INNER JOIN user u ON u.intUid = uhr.user_intUid INNER JOIN role r ON r.intRoleID = uhr.role_intRoleID INNER JOIN employee e ON e.intEmpID = u.employee_intEmpID  WHERE varEmail= '$email' and passWord = '$pwd'");
